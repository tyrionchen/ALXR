...
use cmake::{Config};
use target_lexicon::{Environment, Architecture, ArmArchitecture, OperatingSystem, Triple };
...
fn android_abi_name(target_arch: &target_lexicon::Triple) -> Option<&'static str> {
    match target_arch.architecture {
        ...
        Architecture::Arm(ArmArchitecture::Armv7a) |
        Architecture::Arm(ArmArchitecture::Armv7) => { Some("armeabi-v7a") }
...
fn main() {
    ...
    let file_ext_filters = vec!["h", "hpp", "inl", "c", "cc", "cxx", "cpp", "cmake", "in", "gradle", "pro", "properties"]
        .into_iter()
        .map(OsStr::new)
        .collect::<Vec<_>>();
    ...
    let xr_engine_output_dir = if is_android_env(&target_triple) {
        ...
        let status = Command::new(gradle_cmd_path)
            .arg(format!("-PbuildDir={}", gradle_output_dir.to_string_lossy()))
        ...
        let bin_dir_rel = PathBuf::from(
            format!("intermediates/library_and_local_jars_jni/{0}/jni/{1}", profile, android_abi_name(&target_triple).unwrap()));
        gradle_output_dir.join(&bin_dir_rel)
    } else {
        let default_generator = "Ninja";
        let cmake_generator = env::var("ALVR_CMAKE_GEN")
            .map(|s| if s.is_empty() { String::from(default_generator) } else { s })
    ...
    let defines = if is_android_env(&target_triple) { "-DXR_USE_PLATFORM_ANDROID" } else  { "" };
    ...
    if is_android_env(&target_triple) {
        println!("cargo:rustc-link-search=native={0}", xr_engine_output_dir.to_string_lossy());
        ...
        println!("cargo:rustc-link-search=native={0}", xr_engine_bin_dir.to_string_lossy());
        println!("cargo:rustc-link-search=native={0}", xr_engine_lib_dir.to_string_lossy());
    };
